...
class KNODEDoubleGyre(DoubleGyre):
    ...
    def forward(self, t, y):
        NNOut = self.net(y)
        KOut = self.DoubleGyreDynamics(y.T, t, self.eps, self.alpha, self.A, self.omega)
        b = torch.vstack((KOut[0], KOut[1])).T
        return b + NNOut
...
class KNODEBickleyJet(BickleyJet):
    ...
    def forward(self, t, y):
        NNOut = self.net(y)
        KOut = self.BickleyJetDynamics(y.T, t, self.U0, self.L0, self.eps1, self.eps2, self.eps3, self.R0, self.Dphix, self.Dphiy)
        b = torch.vstack((KOut[0], KOut[1])).T
        return b + NNOut
